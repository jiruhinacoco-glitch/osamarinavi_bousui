/* スマホの数値入力＝自前のキーパッド（§307）
   ...
   ★2026-09-26p スマホは全ページ共通の数字パッド（nn_numpad.js・#nnNumpad）にそろえた。キーは 0-9・.・⌫・クリア・±・次へ・確定 の16個＋見出しの✕（取り消し） */
...
if(M==='pc'){
  const r=await p.evaluate(()=>{ nnNumAsk('てすと',300,()=>{});
    return {old:!!document.querySelector('#nnNumDlg.open'), pad:!!document.querySelector('#nnNumpad.open')}; });
...
const r=await p.evaluate(()=>new Promise(res=>{
  ...
  setTimeout(()=>{
    const e=document.getElementById('nnNumpad'), R=e.getBoundingClientRect();
    const ks=[].map.call(e.querySelectorAll('.np-g button'),b=>({k:b.dataset.k,
    ...
    res({open:e.classList.contains('open'), oldDlg:kb, focus:document.activeElement&&document.activeElement.tagName,
      ...
      val:e.querySelector('.np-v').textContent, vw:innerWidth, vh:innerHeight});
  }, 350);
}));
...
ok(r.keys===16, '② キーは16個（0-9・.・⌫・クリア・±・次へ・確定）', r.keys);
...
/* 打って OK */
await p.evaluate(()=>{ const e=document.getElementById('nnNumpad');
  ...
  hit('c'); hit('4'); hit('5'); hit('0'); hit('ok');
});
await p.waitForTimeout(200);
const got=await p.evaluate(()=>({v:window.__got, open:!!document.querySelector('#nnNumpad.open')}));
...
await p.evaluate(()=>{ const e=document.getElementById('nnNumpad');
  e.querySelector('button[data-k="x"]').dispatchEvent(new PointerEvent('pointerdown',{bubbles:true,cancelable:true,pointerType:'touch',pointerId:4})); });
await p.waitForTimeout(200);
ok(await p.evaluate(()=>window.__got===null), '④ ✕＝取り消し（null）');
/* ⌫と± */
