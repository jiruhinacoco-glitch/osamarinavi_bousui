...
(async()=>{
 ...
 let q=await pg.evaluate(()=>[...document.querySelectorAll('#f_areasBox .arr')].map(r=>r.dataset.k+(r.offsetParent?'':'(隠)')));
 ok(q.join()==='平場,立上り,屋根内周','既定は平場・立上り・屋根内周',q);
 await pg.fill('#f_name','テスト部位物件');
 const ins=await pg.$$('#f_areasBox .arr input'); await ins[0].fill('300'); await ins[1].fill('45.5'); await ins[2].fill('120');
 q=await pg.evaluate(()=>({m:document.getElementById('f_m').value,ro:document.getElementById('f_m').readOnly,sum:document.querySelector('#f_areasBox .arsum').textContent}));
 ok(q.m==='345.5'&&q.ro,P+'㎡の合計が数量に入り手入力不可',q);
 const bt=await pg.evaluate(()=>{const r=document.querySelector('#f_areasBox .aradd').getBoundingClientRect();return {x:r.left+r.width/2,y:r.top+r.height/2}});
 ...
 const ins2=await pg.$$('#f_areasBox .arr input'); await ins2[3].fill('20');
 q=await pg.evaluate(()=>({k:[...document.querySelectorAll('#f_areasBox .arr')].map(r=>r.dataset.k),m:document.getElementById('f_m').value}));
 ...
 // はみ出し
 q=await pg.evaluate(()=>{const m=document.querySelector('#modalbg .modal').getBoundingClientRect();return [...document.querySelectorAll('#f_areasBox *')].filter(e=>{const r=e.getBoundingClientRect();return r.width&&r.right>m.right-2}).length});
