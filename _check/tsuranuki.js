/* ★2026-09-09b §366 予告線が躯体を貫通していないか（本人の指示
   「立上り、天端、平場、貫通するときがある。線を貫通させる概念をなくして」）。
   モデルの中身は使わず、カメラから光線を撃って「線の点が躯体の裏に隠れていないか」で測る。
   直す前の版（git show HEAD:zumen_sekisan.html）では ★NG が出る。 */
...
(async()=>{
  ...
  for(const tgt of [[9.0,0.312,7.87],[9.0,0.15,7.738],[9.0,0.312,7.62]]){
    ...
    const ng=(r.seg<1)||(r.bad>0);
    if(ng) NG++;
    console.log((ng?'★NG ':'○ ')+'狙い'+JSON.stringify(tgt)+' → 線'+r.seg+'本  躯体の裏に隠れている点 '+r.bad+'/'+r.tot+(r.worst?('  例'+JSON.stringify(r.worst)):''));
    await p.evaluate(`(${TOUCH})('pointercancel',61,${f.x},${f.y})`); await p.waitForTimeout(120);
  }
  if(errs.length){ NG++; console.log('★NG ページのエラー '+errs.slice(0,2).join(' / ')); }
  console.log(NG?('★NG '+NG+'件'):'○ 予告線は躯体を貫通していない');
  await b.close();
})();
