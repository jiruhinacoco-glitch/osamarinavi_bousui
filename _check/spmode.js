...
(async()=>{
 ...
 /* ---- B. スマホモード（縦）：レイアウト ---- */
 {
  ...
  ok(card.ttl>=15&&card.ttlWrap==='normal'&&card.go>=34&&card.goIn&&card.photoW>=100&&card.ovf===0,'スマホモード：物件名15px以上で折返し可・詳細ボタン34px以上（§445で詰めた）・写真110px・はみ出しなし '+JSON.stringify(card));
