/* 始点の自由配置と、非整数の始点から閉じた完成形を実操作で検査。 */
...
(async()=>{
 ...
 for(const scale of [1,.25,5]){
  ...
  ok(first&&Math.abs(first.x-2.37)<.03&&Math.abs(first.y-1.63)<.03,'1マス='+scale+'m：始点をクリック位置に置く',first);
  for(const [x,y]of [[6.37,1.63],[6.37,5.63],[2.37,5.63],[2.37,1.63]])await click(x,y);
  const shape=await p.evaluate(()=>state.polys[0]?.pts);
  ok(shape?.length===4&&shape.every((q,i)=>Math.hypot(q.x-[2.37,6.37,6.37,2.37][i],q.y-[1.63,1.63,5.63,5.63][i])<.04),'非整数の始点で4辺を閉じた完成形',shape);
 }
 await p.reload();await p.evaluate(()=>nnZMenuClose());
 const saved=await p.evaluate(()=>state.polys[0]?.pts);
 ok(saved?.length===4&&Math.abs(saved[0].x-2.37)<.03&&Math.abs(saved[0].y-1.63)<.03,'開き直しても始点がマスへ丸まらない',saved);
