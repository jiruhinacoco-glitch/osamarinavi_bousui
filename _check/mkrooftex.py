...
# ① 改質アスファルトシート 砂付（新品）
#    ★2026-09-05f 本人の指摘「施工後の防水層がこんなに汚い。おかしい」。
#      直す前は 汚れのたまり（blob）・雨だれの筋（flow）・30cm〜1.4mのムラ を入れていたので、
#      新品なのに「風化したコンクリート」に見えていた。
#      **新品に汚れ・雨だれ・大きなムラは無い。** 入れるのは
#      ①砂粒（mm）②敷きならしのごく弱いムラ、の2つだけ。凸凹も砂粒だけにする
#      （10〜30cmのうねりを凸凹に入れると、光の当たり方が荒れて汚れて見える）。
def as_new():
    g    = band(1.0, 2.2, 11)                 # 砂粒（近寄ったとき）
    g2   = band(2.5, 6.0, 12)                 # 粒のかたまり
    mid  = band(14, 40, 13)                   # 敷きならしのムラ（ごく弱く）
    t = np.clip(0.5 + 0.34*g + 0.19*g2 + 0.05*mid, 0, 1)
    col = mixc(hx('6b6862'), hx('939086'), t)                 # 砂付の灰褐色（新品はくすみが無い）
    col += np.dstack([band(1,2.2,21), band(1,2.2,22), band(1,2.2,23)]) * 4.0
    h = 1.00*g + 0.42*g2                      # 凸凹は砂粒だけ（うねりを入れない）
    r = np.clip(0.90 - 0.05*g + 0.02*g2, 0.78, 0.98)
