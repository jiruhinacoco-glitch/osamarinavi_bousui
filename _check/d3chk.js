...
(async()=>{
  ...
  p.on('response',r=>{ if(r.status()>=400 && !/icon_zumen\.png|\/icons\/btn_|\/icons\/ago_|\/icons\/kz_|\/icons\/zm_|\/icons\/kizon_|\/icons\/dk_|\/icons\/kou_/.test(r.url())) warns.push(r.status()+' '+r.url()); });
  ...
  /* ★2026-09-04g パソコンの入口メニューは、開いている間だけ three.js を読んで小さな模型を回す。
     ここではメニューを閉じてから測っている＝「閉じたら読まない」ことの確認になる */
  ok('ページを開いた時点（メニューを閉じた直後）では three.js を読まない', net.length===0, JSON.stringify(net));
  ...
  /* ★2026-09-02b 背景は「空の絵」（§265③）。★2026-09-04i 夜は廃止＝夜画面でも光はそのまま（標準） */
  const dk=await p.evaluate(()=>({sky:(window.nnSkyKind?nnSkyKind():''),
    bgTex:!!(T.scene.background&&T.scene.background.isTexture), keys:Object.keys(NN_SKY).join(',')}));
  ok('夜画面でも背景は空の絵のまま・光は「標準」（夜は無い）', dk.sky==='std' && dk.bgTex && !/yoru/.test(dk.keys), JSON.stringify(dk));
