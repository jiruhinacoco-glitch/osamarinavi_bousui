...
try{for(const cfg of [{w:1440,h:900,z:1},{w:1920,h:920,z:1.4},{w:390,h:844,z:1,phone:true},{w:844,h:390,z:1,phone:true}]){
 ...
 const page=await ctx.newPage(),errors=[];page.on('pageerror',e=>errors.push(e.message));await page.goto('http://a1.test/kokkosho.html');await page.evaluate(z=>{document.body.style.zoom=z;NN_A1.open();},cfg.z);
 ...
 const coats=await page.evaluate(()=>{const gs=document.querySelector('#a1-dialog')._a1.groups;let n=0,wave=0;for(let i=3;i<=8;i++){for(const m of gs[i].children){if(m.material.userData.kind==='asphalt'){n++;const p=m.geometry.attributes.position;if(i===7){const xs=[];for(let k=0;k<p.count/2;k+=(document.documentElement.dataset.nnphone==='1'?17:25))xs.push(p.getX(k));wave=Math.max(...xs)-Math.min(...xs);}}}}return {n,wave};});check(coats.n===10&&coats.wave>.02,'流し張り4工程の平場・立上り＋上掛け2回に波状端のアス層');
