...
(async()=>{
 ...
 for(const scale of [1,.25,5]){
  ...
  ok(first&&first.x===3&&first.y===2,'1マス='+scale+'m：始点をマス目の交点に合わせる',first);
  for(const [x,y]of [[1.89,5.77],[7.41,6.13],[8.03,2.27],[3.37,1.63]])await click(x,y);
  const shape=await p.evaluate(()=>state.polys[0]?.pts);
  ok(shape?.length===4&&shape.every((q,i)=>Math.hypot(q.x-[3,1.89,7.41,8.03][i],q.y-[2,5.77,6.13,2.27][i])<.04),'任意角度の4辺を閉じた完成形',shape);
 }
 await p.reload();await p.evaluate(()=>nnZMenuClose());
 const saved=await p.evaluate(()=>state.polys[0]?.pts);
 ok(saved?.length===4&&saved.every((q,i)=>Math.hypot(q.x-[3,1.89,7.41,8.03][i],q.y-[2,5.77,6.13,2.27][i])<.04),'開き直しても全頂点が狙った位置を維持',saved);
