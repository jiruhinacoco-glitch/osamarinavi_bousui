   ...
   上の帯の一言（図面をかく → 数量・見積を自動算出）は削除 */
{
  ...
  /* ★2026-09-06g スマホは幅が足りないので見出しを「対応可能な機能」に短縮した（§306） */
  ok(MOB ? t.ftt.join('/')==='対応可能な機能/対応可能な機能'
        : t.ftt.join('/')==='平面図作成で対応可能な機能/矩計図作成で対応可能な機能',
     '「対応可能な機能」の見出しが出る',t.ftt);
  ok(t.fts.slice(0,3).join('/')==='3D投影/積算＆見積/割付図',
     '機能の名前（3D投影／積算＆見積／割付図）',t.fts.slice(0,3));
  ok(!/図面をかく/.test(t.hco),'上の緑帯の一言は削除されている',t.hco);
}
ok(n.foot===3,'下の行が3つ（保存データ一覧・初期詳細設定・地図から平面図を作成）',n.foot);
...
if(M==='land'){
  ...
  ok(big.card>=150 && big.card<=620,'よこ向き：①②のカードの背が妥当（150〜620px・2026-09-05e タイトル1.2倍＋3D模型。メニューはスクロールする）',big.card+'px');
  ok(big.ttl>=12 && big.ttl<=17,'よこ向き：タイトルは12〜17px（2026-09-05e 1.2倍）（縮小後の決まり）',big.ttl+'px');
}
