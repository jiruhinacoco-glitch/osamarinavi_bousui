...
ok(v.buttons===10&&!v.cta,'詳細設定10件・青い説明削除',v.buttons);ok(v.radius==='3px'&&+v.bold>=800&&parseFloat(v.line)>=.8&&parseFloat(v.line)<=1.1,'角・現場名太字・縦罫線',v);
...
if(v.buttons===10){
 for(let i=0;i<10;i++){await p.locator('.dash-settings-btn').nth(i).click();ok(await p.locator('#dashSettings').isVisible(),'設定を開く '+i);await p.locator('#dashSettingSize').selectOption('large');await p.locator('#dashSettingOption').uncheck();await p.locator('#dashSettingSave').click();await p.waitForFunction(()=>document.querySelectorAll('.dash-settings-btn').length===10);}
 await p.reload();await p.waitForFunction(()=>document.querySelectorAll('.dash-settings-btn').length===10);ok(await p.locator('.dpanel.dash-large').count()===10,'10パネル設定を再読込で復元');
 await p.locator('.dash-settings-btn').nth(5).click();const cb=p.locator('[data-col]');const last=cb.last();await last.uncheck();await p.locator('#dashSettingSave').click();await p.waitForFunction(()=>document.querySelectorAll('.dash-settings-btn').length===10);await p.reload();await p.waitForFunction(()=>document.querySelectorAll('.dash-settings-btn').length===10);await p.locator('.dash-settings-btn').nth(5).click();ok(!await p.locator('[data-col]').last().isChecked(),'列表示設定の保存復元');await p.locator('#dashSettingCancel').click();
