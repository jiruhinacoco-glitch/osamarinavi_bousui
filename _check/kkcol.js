/* 検査：国交省仕様（スマホ表示・縦）の一覧で、行ごとに列の位置がズレない（2026-09-26h）
   ...
   ・記号・種別の文字が絵に重ならない（B-1（砂付あなあき）など・2026-09-26j）
...
(async()=>{
  ...
  document.querySelectorAll('table.sp-tbl').forEach(t=>{
   ...
   t.querySelectorAll('tbody tr:not(.gr)').forEach(tr=>{
    ...
    out.push({code:c.textContent.trim().slice(0,8),metL:Math.round(img.getBoundingClientRect().left),tblR:Math.round(b.getBoundingClientRect().right),
     codeR:Math.round(Math.max(...[...c.childNodes].map(n=>{const g=document.createRange();g.selectNodeContents(n);return Math.max(0,...[...g.getClientRects()].map(q=>q.right));}))),tops:[c,m,b].map(e=>Math.round(e.getBoundingClientRect().top)),head:Math.round(h2),metCell:Math.round(m.getBoundingClientRect().left)});
   });
  });
  return out;});
 ...
 const ovBad=r.filter(x=>x.codeR>x.metL-2);
 ok(ovBad.length===0,'記号・種別の文字が工法の絵に重ならない '+JSON.stringify(ovBad.slice(0,3).map(x=>[x.code,x.codeR,x.metL])));
