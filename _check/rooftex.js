/* ★2026-09-02a 屋根（防水層）の質感を本物にした（§255）の検証
   ...
   ・1タイル＝砂付1.0〜1.2m／他8m。色は白（掛け算で沈ませない）
...
if(m.h){
  ...
  /* ★2026-09-08v §342 砂付だけ 1タイル1.0〜1.2m（粒が1〜2mmなので、8mでは画素より小さくて粒に見えない） */
  ok('1タイル＝砂付は1.0m・他は8m（作った側と同じ）', Math.abs(tile-1)<0.01, tile.toFixed(2)+'m');
...
try{
  ...
  ok('施工後は既存防水より明るい', nw[1]>ag[1]+12, nw[1]+' / '+ag[1]);
