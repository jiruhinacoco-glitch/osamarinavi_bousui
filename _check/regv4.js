...
(async()=>{
 ...
 q=await pg.evaluate(()=>{const rf=document.querySelector('#f_kouji .rf');const arr=[...rf.querySelectorAll('.arr')];
   return {tk:[...rf.querySelectorAll('.tkseg button')].map(b=>b.textContent+(b.classList.contains('on')?'*':'')),hiraKob:getComputedStyle(arr[0].querySelector('.kob')).visibility,alt:!!rf.querySelector('.rfalt')};});
 ok(q.tk.join()==='かぶせ*,部分撤去,全面撤去'&&q.hiraKob==='hidden'&&!q.alt,'改修：既存防水の扱い（はじめはかぶせ）・平場に工法ボタンなし・別工法の行なし',q);
