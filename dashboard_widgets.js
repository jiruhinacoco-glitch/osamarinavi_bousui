...
(function(){
 ...
 html[data-nnphone="1"] #dashboard .nn-widget-move,html[data-nnphone="1"] #dashboard .nn-widget-edge{display:none!important;}
 `;document.head.appendChild(style);
 const mode=()=>document.documentElement.dataset.nnvm||'pc';
 /* ★2026-09-25b スマホでは表・グラフの大きさと位置を変えない（保存済みの値も使わない）。指で触れて崩れ、戻せなくなるため */
 const PH=document.documentElement.getAttribute('data-nnphone')==='1';
 function conf(w){return PH?{}:(prefs[mode()+'|'+w.dataset.widgetKey]||{});}
 ...
 function applyPayment(w){if(!paymentParents.has(w.parentElement)){paymentParents.add(w.parentElement);paymentResize.observe(w.parentElement);}let c=conf(w);if(c.flowVersion!==1){c={...c,flowVersion:1};delete c.x;delete c.y;delete c.height;prefs[mode()+'|'+w.dataset.widgetKey]=c;save();}const parent=w.parentElement;if(!parent.clientWidth)return;w.style.position='relative';w.style.left='';w.style.top='';parent.style.height='';parent.classList.remove('nn-widget-canvas');const width=Math.max(Math.min(280,parent.clientWidth),Math.min(parent.clientWidth,Number.isFinite(c.width)?c.width:parent.clientWidth<600?parent.clientWidth:parent.clientWidth*(w.dataset.widgetKey==='nyukin-table'?.6:.4)-8));w.style.width=width+'px';if(Number.isFinite(c.height)){w.style.height=Math.max(w.dataset.widgetKey==='nyukin-calendar'?360:180,Math.min(1000,c.height))+'px';w.classList.add('nn-widget-sized');}else{w.style.height='';w.classList.remove('nn-widget-sized');}}
 ...
 const paymentParents=new WeakSet(),paymentResize=new ResizeObserver(es=>es.forEach(e=>{if(e.contentRect.width>0)e.target.querySelectorAll(':scope>.nn-widget').forEach(applyPayment);}));
