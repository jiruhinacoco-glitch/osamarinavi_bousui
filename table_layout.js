...
(function(){
 ...
 function decorate(t){if(!t.rows.length)return;let state=known.get(t);if(!state){t.dataset.tableLayoutKey=key(t);state={};known.set(t,state);wire(t);}
  for(const th of t.querySelectorAll('th')){const sort=th.querySelector(':scope>.sortbtn');if(sort){const wrap=document.createElement('span');wrap.className='nn-th-label';const nodes=[...th.childNodes].filter(n=>n===sort||n.nodeType===3);th.insertBefore(wrap,nodes[0]||th.firstChild);nodes.forEach(n=>wrap.appendChild(n));}}
 ...
 function wire(t){t.addEventListener('pointerdown',e=>{
  ...
  function move(ev){if(!drag&&Math.hypot(ev.clientX-startX,ev.clientY-startY)<7)return;if(!drag){drag=true;cell.setPointerCapture(e.pointerId);}ev.preventDefault();const hit=document.elementFromPoint(ev.clientX,ev.clientY)?.closest(isCol?'th':'td');if(hit&&hit.closest('table')===t&&hit!==cell){target?.classList.remove('nn-cell-drop');target=hit;target.classList.add('nn-cell-drop');}}
  function done(ev){document.removeEventListener('pointermove',move,true);document.removeEventListener('pointerup',done,true);document.removeEventListener('pointercancel',done,true);target?.classList.remove('nn-cell-drop');if(cell.hasPointerCapture(e.pointerId))cell.releasePointerCapture(e.pointerId);if(!drag)return;suppress();if(!target||ev.type==='pointercancel')return;
 ...
 let pending=false;function scan(){pending=false;document.querySelectorAll('table').forEach(decorate);}function schedule(){if(!pending){pending=true;requestAnimationFrame(scan);}}new MutationObserver(schedule).observe(document.body,{childList:true,subtree:true});scan();
})();
